...
* If the cluster you want to add is configured with pod security admission enforcement, which is the default for Kubernetes 1.25 and later clusters, you need to enable PSA restrictions on the appropriate namespaces. Refer to link:setup_overview.html#enable-psa[Enable PSA restrictions] for instructions.
