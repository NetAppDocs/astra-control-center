...
NOTE: If the cluster you want to add has has pod security admission enforcement enabled, which is standard for Kubernetes 1.25 and later clusters, you need to enable PSA restrictions on the appropriate namespaces. Refer to link:setup_overview.html#enable-psa[Enable PSA restrictions] for instructions.
