...
[.lead]
Astra Control Center uses Astra Connector to enable communication between Astra Control Service and clusters that it manages. You need to install Astra Connector on all clusters that you want to manage.
...
NOTE: If the cluster is configured with pod security admission enforcement, which is the default for Kubernetes 1.25 and later clusters, you need to enable PSA restrictions on the appropriate namespaces. Refer to link:setup_overview.html#enable-psa[Enable PSA restrictions] for instructions.
