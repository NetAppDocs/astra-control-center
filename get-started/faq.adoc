...
keywords: faq, frequently asked questions, astra, astra control, kubernetes, persistent volumes
...
While you can access Astra Control Service at https://astra.netapp.io, Astra Control Center uses local authentication and a URL specific to each environment.

For the URL, in a browser, enter the Fully Qualified Domain Name (FQDN) you set in the spec.astraAddress field in the astra_control_center_min.yaml CRD when you installed Astra Control Center.
...
.Steps
1. From the left navigation, select *Account* > *Licenses*.
2. Select *Add license*.
3. Browse to the license file you downloaded.
...
+
Alternatively, delete the PVCs and PVs.
.	Delete the cluster.
...
The NetApp Trident operator sets a default storage class after it's successfully added to Astra Control. When an application's PVCs are not bound to XYZ, there are a few steps that you can take:
