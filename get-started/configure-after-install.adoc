...
=== Add a new certificate using the Astra Control Center UI
Add a TLS certificate that is signed by a CA using Astra Control Center UI. This method is useful for adding certificates for authentication of remote connections (such as an LDAP server).
