...
* *Ensure a routable FQDN*: The Astra FQDN you plan to use is able to be routed to the cluster. This means that you either have a DNS entry in your internal DNS server or you are using a core URL route that is already registered.
...
* *Consider a service mesh*: It is strongly recommended that Astra Control host cluster communications channels be secured using an link:requirements.html#service-mesh-requirements[supported service mesh^]. 
+
.Istio service mesh details 
[%collapsible]
====
For Istio service mesh use, you'll be required to add an `istio-injection:enabled` <<Complete Astra Control Center and operator installation,label>> to the Astra namespace as described during this Astra Control Center installation procedure. You'll also need to use the `Generic`  <<Configure Astra Control Center,ingress setting>> and provide an alternative ingress for <<Set up ingress for load balancing,external load balancing>>.
====
// End snippet
...
.. Enter the Astra Control Center version; for example, 23.07.0-25.
