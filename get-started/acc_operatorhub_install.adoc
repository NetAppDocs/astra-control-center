...
[%collapsible]
====
For Istio service mesh use, you'll be required to add an `istio-injection:enabled` label to the Astra namespace prior to deploying Astra Control Center. You'll also need to use the `Generic` ingress setting and provide an alternative ingress for link:../get-started/install_acc.html#set-up-ingress-for-load-balancing[external load balancing].
====
// End snippet
