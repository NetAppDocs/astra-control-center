...
NOTE: Podman commands can be used in place of Docker commands if you are using RedHat’s podman repository.
...
+
----
kubectl get acc -n [netapp-acc or custom]
----
+
...
.Steps
. In a browser, enter the FQDN you used in the `astraAddress` in the  `astra_control_center_min.yaml` CR when <<Install Astra Control Center using the command-line utility,you installed ACC>>.
. Accept the self-signed certificates when prompted.
+
NOTE: You can create a custom certificate after login.

. At the Astra Control Center login page, enter the value you used for `email` in `astra_control_center_min.yaml` CR when <<Install Astra Control Center using the command-line utility,you installed ACC>>, followed by the one-time password (`ACC-[UUID]`).
+
NOTE: If you enter an incorrect password three times, the admin account will be locked for 15 minutes. If you forget the password that was just updated and no other admin or owner user accounts have been created, contact NetApp Support for password recovery assistance.
