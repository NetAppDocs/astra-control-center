...
IMPORTANT: Do not delete the Astra Control Center operator (for example, `kubectl delete -f astra_control_center_operator_deploy.yaml`) at any time during the entire Astra Control Center installation to avoid deleting pods.
...
ls kubectl-astra/
----
. Copy the correct binary and rename it to `kubectl-astra`. Copy the binary to the same location as the standard kubectl utility. In this example, the kubectl utility is located in the `/usr/local/bin` directory. Replace `<binary-name>` with the name of the file you need:
+
[source,console]
----
...
----
+
NOTE: Before you complete installation, be sure your KUBECONFIG is pointing to the cluster where you want to install Astra Control Center. Only one context is permitted for the KUBECONFIG.
...
+
* *Generic* (`ingressType:Generic`)
+
Use this option when you have another ingress controller in use or would prefer to use your own ingress controller. This is the default action with Astra Control Center. After Astra Control Center is deployed, you will need to configure the link:../get-started/install_acc.html#set-up-ingress-for-load-balancing[ingress controller] to expose Astra Control Center with a URL.

* *AccTraefik* (`ingressType:AccTraefik`)
+
Use this option when you would prefer to not configure an ingress controller. This deploys the Astra Control Center `traefik` gateway as a Kubernetes LoadBalancer type service.
+
...
[source,console]
----
openssl req -x509 -nodes -days 7 -newkey rsa:2048 -keyout tls.key -out tls.crt
----
