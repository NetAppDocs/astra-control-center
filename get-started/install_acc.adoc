...
----
.. If you do not use a registry that requires authentication, delete these lines after `imagePullPolicy`:
+
----
imagePullSecrets:
...
.  If you use a registry that requires authentication, you need to create a secret for the `netapp-acc-operator` namespace. Run the following command:
+
----
oc create secret docker-registry astra-registry-cred -n netapp-acc-operator --docker-server=<ASTRA_IMAGE_REGISTRY> --docker-username=<username> --docker-password=<token>
----
+
Sample response:
+
...
.  If you use a registry that requires authentication, you need to create a secret for the `netapp-acc` namespace. Run the following command:
+
----
oc create secret docker-registry astra-registry-cred -n netapp-acc --docker-server=<ASTRA_IMAGE_REGISTRY> --docker-username=<username> --docker-password=<token>
----
+
Response
+
