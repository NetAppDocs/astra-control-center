...
+
This deploys the Astra Control Center `traefik` gateway as a Kubernetes LoadBalancer type service. You also need to provide an external load balancer (such as MetalLB) for Astra Control Center to get an external IP.
+
* If you want to use an ingress, see link:../get-started/install_acc.html#set-up-ingress[Set up ingress].
