...
You can configure and install the operator YAML using either the commands described here or by completing the installation from Operatorhub.io.

.Options
* link:../get-started/acc_operatorhub_install.html[Install from Operatorhub.io]
* <<Install the operator using the command line,Install using command line>>

=== Install the operator using the command line
...
. Run the following patch to correct link:../release-notes/known-issues.html#Incorrect-ClusterRoleBinding-created-by-Astra-Control-Center-CRD-during-installation[cluster role binding].
