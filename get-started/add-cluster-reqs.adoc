...
----
. Create a PodSecurityPolicy for the cluster that is less restrictive than the default, so that Astra Control Center can create and update pods:
+
----
...
spec:
  ...
  fsGroup:
    rule: 'RunAsAny'
----
. Create a new role for the PodSecurityPolicy:
+
----
