...
If no volume snapshot class is configured on the cluster when you add it, Astra Control Center attempts to configure one with a default configuration. If the volume snapshot class installation fails, you need to manually install it. See the Astra Trident https://docs.netapp.com/us-en/trident/trident-use/vol-snapshots.html[instructions^] to enable and test Volume Snapshots with Astra Trident.
