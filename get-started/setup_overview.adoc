...
You can update existing backends to move from credential-based to certificate-based methods. However, only one authentication method is supported at a time. 

=== Enable credential-based authentication

Astra Control Center requires the credentials to an SVM-scoped/cluster-scoped `admin` to communicate with the ONTAP backend. You should use standard, pre-defined roles such as `admin` or `vsadmin`. This ensures forward compatibility with future ONTAP releases that might expose feature APIs to be used by future Astra Control Center releases. 
