...
You can add a storage backend so that Astra Control can manage its resources. You can deploy a new storage backend on a managed cluster or use an existing storage backend.
...
* You have link:../get-started/setup_overview.html#add-cluster[added a Kubernetes cluster] and it is managed by Astra Control.
+
NOTE: The managed cluster has a supported backend attached to it that can be discovered by Astra Control.

* (Astra Data Store installations only) You have added your Kubernetes app cluster.
+
...
.. Select *Manage*.
. Choose the new deployment option:
.. Select the *Astra Data Store* option within the *Deploy New* tab.
. Add the Astra Data Store package:
.. Select *Add packages* and select the version of Astra Data Store you want to deploy.
...
+
NOTE: Astra Data licenses with full permissions are associated with your Kubernetes cluster, and these associated cluster should appear automatically. If there is no managed cluster, you can select the Add a Cluster option to add one to Astra Control management.
+
...
. (Astra Data Store preview licenses only) If you have not added a Kubernetes cluster to Astra Control management or selected the cluster that should be associated with your preview license, you need to do so from the *Kubernetes cluster* page. Select an existing cluster from the list or select *add the underlying cluster* to add a cluster to Astra Control management.
. Select the deployment template size for the Kubernetes cluster that will provide resources for Astra Data Store.
. Configure the nodes:
.. (Optional) Add a node label to identify the pool of worker nodes that supports this Astra Data Store cluster.
...
. (Astra Data Store full licenses only) Enter the key of the label you want to use for Protection Domains rack failures.
//Super unclear in UI as is.
...
//Need instructions for getting list of NICs using CLI.
. Review the configuration overview and the configuration as a custom resource. Select *Deploy storage backend* to begin installation.
+
After a successful installation, the backend appears with a status of `healthy` in the list along with active performance information.
