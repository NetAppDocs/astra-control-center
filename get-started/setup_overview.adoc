...
.What you'll need for Astra Data Store deployments
...
//add reference to package management topic

* You have uploaded the Astra Data Store license for you deployment to a location that is accessible to Astra Control.
...
.Result
After a successful installation, the backend appears with a status of `healthy` in the backends list along with active performance information.
