...
If you have a third-party certificate, you can set up authentication with these steps. 
...
+
----
openssl req -new -newkey rsa:4096 -nodes -sha256 -subj "/" -outform pem -out aff250-astra-01-02.csr -keyout aff250-astra-01-02.key -addext "subjectAltName = DNS:aff250-astra-01-02-cm.rtp.openenglab.netapp.com,IP:10.192.96.13,DNS:aff250-astra-01-02"
----

. Pass the CSR to the Windows CA (third-party CA) and issue the signed certificate. 

. Download the signed certificate and name it `aff250-astra-01-02.pem'
...
+
* *Private key*: `aff250-astra-01-02.key` (This is the corresponding key for the server certificate in ONTAP. It is needed while installing the server certificate.)

* *Signed certificate*: `aff250-astra-01-02.crt` (This is also called the _server certificate_ in ONTAP.)

* *Root CA certificate*: `ca_root.crt` (This is also called the  _`_server-ca certificate_ in ONTAP.)
...
aff250-astra-01-02::> security certificate install -type server-ca
...
# Copy the contents of aff250-astra-01-02.crt and use it here. For key, use the contents of aff250-astra-01-02.key file. 
 
aff250-astra-01-02::> security certificate install -type server
...
Enter a valid common name to continue installation of the certificate: aff250-astra-01-02-cm.rtp.openenglab.netapp.com
...
aff250-astra-01-02::> ssl modify -vserver aff250-astra-01-02 -ca <CA>  -server-enabled true -serial <serial number>       (security ssl modify)
...
msaravan@msaravan-mac-1 /Users/msaravan/projects/cert/feb14 % openssl s_client -CAfile ca_root.crt -showcerts -servername server -connect aff250-astra-01-02-cm.rtp.openenglab.netapp.com:443
CONNECTED(00000005)
...
openssl req -x509 -nodes -days 1095 -newkey rsa:2048 -keyout aff250-astra-01-02-client.key -out aff250-astra-01-02-client.pem -subj "/CN=admin"
 
ls

<key numbers>-client.key 
<CSR name>.csr    
<CRD details>.crt
<Client details>-client.pem 
<key details>.key    
ca_root.crt

Copy the content of <client details>-client.pem  file and use it in the below command:
 
aff250-astra-01-02::> security certificate install -type client-ca -vserver aff250-astra-01-02
...
ssl modify -vserver  aff250-astra-01-02 -client-enabled true
  (security ssl modify)
 
 
# Setting permissions for certificates
security login create -user-or-group-name admin -application ontapi -authentication-method cert -role admin -vserver aff250-astra-01-02

security login create -user-or-group-name admin -application http -authentication-method cert -role admin -vserver aff250-astra-01-02
 
==
 
#Verfiy passwordless communication works fine with the use of only certificates:
 
curl 
--<crt details>.crt 
--key <key details>-client.key 
--cert <client details>-client.pem 
<url>
...
* *Key*: `aff250-astra-01-02-client.key`
* *Pem*: `aff250-astra-01-02-client.pem`
* *Crt*: `aff250-astra-01-02.crt`
