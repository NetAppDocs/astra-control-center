...
* *Credential-based*: The username and password to an ONTAP user with the required permissions. You should use a pre-defined security login role, such as admin or vsadmin to ensure maximum compatibility with ONTAP versions.

* *Certificate-based*: Astra Control Center can also communicate with an ONTAP cluster using a certificate installed on the backend. Here, the backend definition must contain Base64-encoded values of the client certificate, key, and the trusted CA certificate if used (recommended).

You can update existing backends to move from credential-based to certificate-based methods. However, you cannot change from certificate-based authentication to credentials-based authentication. Only one authentication method is supported at a time. 
...
The backend definition is the only place the credentials are stored in plain text. After the backend is created, usernames and passwords are encoded with Base64 and stored as Kubernetes secrets. The creation or update of a backend is the only step that requires knowledge of the credentials. As such, it is an admin-only operation, to be performed by the Kubernetes or storage administrator.
...
. Add a trusted CA certificate to the ONTAP cluster. This might be already handled by the storage administrator. Ignore if no trusted CA is used.
...
. Confirm that the ONTAP security login role supports cert authentication method.
...
. Test authentication using certificate generated. Replace <ONTAP Management LIF> and <vserver name> with the Management LIF IP and SVM name. You must ensure the LIF has its service policy set to default-data-management.
...
. Add the storage backend using the values obtained from the previous step in the Astra Control Center UI.

=== Add the storage backend in the Astra Control Center UI
After you set up the credentials or certificate information, you can add an existing ONTAP storage backend to Astra Control Center to manage its resources.
...
. Select *Use existing* and select *ONTAP*. 
...
* *Use a certificate*: Upload the certificate `.pem` file, the certificate key `.key` file, and optionally the certificate authority  `.pem` file.
