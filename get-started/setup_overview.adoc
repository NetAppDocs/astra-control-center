...
.. Create a `ClusterRole` file called `astracontrol-clusterrole.yaml`.
+
Adjust the name and namespace as needed. If changes are made here, you should apply the same changes in the following steps.
+
[source]
...
[source,console]
----
kubectl apply -f astracontrol-clusterrole.yaml
----
...
+
IMPORTANT: If you create your own `kubeconfig` file, you should define only *one* context element in it. Refer to https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/[Kubernetes documentation^] for information about creating `kubeconfig` files. If you created a kubeconfig for a limited cluster role using <<Create an admin-role kubeconfig,the process above>>, be sure to upload or paste that kubeconfig in this step.
