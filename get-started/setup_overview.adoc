...
New and existing backends can use a certificate and communicate with the ONTAP backend. You should enter the following information in the backend definition.
...
. Install the client certificate of type `client-ca` and key (from step 1) on the ONTAP cluster.
