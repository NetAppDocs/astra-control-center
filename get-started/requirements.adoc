...
To integrate Astra Control Center with the Istio service mesh installed on the host cluster, you must do the integration as part of an Astra Control Center link:../get-started/install_acc.html[installation] or link:../get-started/install_acc.html[upgrade] and not independent of these processes.
...
== Image registry
You must have an existing private Docker image registry to which you can push Astra Control Center build images. You need to provide the URL of the image registry where you'll upload the images.
...
* *Application cluster requirements*: link:../get-started/prep-for-cluster-management[Cluster management requirements]
