...
|9090
|HTTPS
|Ingress (to Astra Control Center cluster), Egress (from the node IP address of each worker node of each managed cluster)
|Metrics data to Metrics consumer - ensure each managed cluster can access this port on the Astra Control Center cluster
