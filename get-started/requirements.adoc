...
If you install Istio service mesh on the host cluster, configure generic ingress and provide an alternative ingress service during Astra Control Center installation. Refer to https://docs.netapp.com/us-en/astra-control-center/get-started/install_acc.html#set-up-ingress-for-load-balancing[Set up ingress for load balancing^] for instructions.
