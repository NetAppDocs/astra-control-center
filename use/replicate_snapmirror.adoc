...
* *Peering*: 
** *Cluster and SVM*: ONTAP clusters and the host SVM must be peered. Refer to https://docs.netapp.com/us-en/ontap-sm-classic/peering/index.html[Cluster and SVM peering overview^] for more information.
** *Astra Trident and SVM*: The peered remote SVM must be available to Astra Trident on the destination cluster.
