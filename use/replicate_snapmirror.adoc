...
* *Astra Trident / ONTAP configuration*: Astra Control Center requires that you configure at least one storage class that supports replication for both source and destination clusters. If the source and destination clusters are the same, the source application must use a different storage class than the destination application.
...
* *Destination storage class*: Select or enter the storage class that uses the paired SVM on the destination ONTAP cluster. The destination storage class must be different than the source storage class.
