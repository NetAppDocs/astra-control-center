...
. If you have a registry that uses secrets, create a secret to use to pull the Astra Control Provisioner image:
+
----
kubectl create secret docker-registry <secret-name> -n trident --docker-server=<your_registry> --docker-username=<username> --docker-password=<token>
----
