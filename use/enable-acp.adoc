...
. Add the secret to the Astra Trident orchestrator:
+
----
...
spec:
  debug: true
  namespace: trident
  tridentImage: <custom registry>/trident:23.07.0
  imagePullSecrets:
  - <secret name>
----
--
...
.. Set the registry location for the Astra Trident image (`tridentImage: <my_custom_registry>/trident:23.10.0`).
.. Set the registry location for the Astra Control Provisioner image (`acpImage: <my_custom_registry>/trident-acp:23.10.0`).
.. If you established <<pull-secrets,image pull secrets>> earlier in this procedure, you can set them here (`imagePullSecrets: - <secret name>`). Use the same name secret name you established in the previous steps.
...
spec:
  ...
  *tridentImage: <my_custom_registry>/trident:23.10.0*
  *imagePullSecrets:
  - <secret name>*
----

. Save the file. The deployment process will begin automatically.
