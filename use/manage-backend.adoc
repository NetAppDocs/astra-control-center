...
== Update a storage backend license
You can update the license for a storage backend to support a larger deployment or enhanced features.

.What you'll need

* A deployed and managed storage backend
* A license file (contact your NetApp sales representative to purchase a license, if needed)
...
|At least one license has been added.
a|

Select a license from the list.

|No licenses have been added.
a|
...
== Upgrade a storage backend
You can upgrade a backend from within Astra Control Center. To do so, you must first upload an upgrade package; Astra Control Center will use this upgrade package to upgrade the backend storage.

.What you'll need
* A managed storage backend
* An uploaded upgrade package (see link:manage-packages-acc.html[Manage software packages])

.Steps

. Select *Backends*.
. Choose a storage backend from the list, and select the corresponding menu in the *Actions* column.
. Select *Upgrade*.
