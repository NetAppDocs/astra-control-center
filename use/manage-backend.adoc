...
. Select the storage backend.
. At the Authentication field, select the *Edit* option. 
. In the Edit Authentication page, select one of the following: 
. Select one of the following. If the certificate is already enabled, you can only edit the certificate information. 

+
* *Update administrator credentials*: Enter the ONTAP cluster management IP address and admin credentials. The credentials must be cluster-wide credentials. 
+
...
* *Change to certificate authentication*: Upload the certificate `.pem` file, the certificate key `.key` file, and optionally the certificate authority  `.pem` file.
