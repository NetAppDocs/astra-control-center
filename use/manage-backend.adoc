...
. Select the storage backend.
. At the Credentials field, select the *Edit* option. 
. In the Edit page, select one of the following. 

//If the certificate is already enabled, you can only edit the certificate information. You cannot change from certificate-based to credentials-based authentication. 
...
* *Change to certificate authentication*: Upload the certificate `.pem` file, the certificate key `.key` file, and optionally the certificate authority file.
