...
== Rotate or remove bucket credentials
Astra Control uses bucket credentials to discover a Kubernetes cluster and the apps on the cluster, and to provision resources on your behalf.

=== Rotate bucket credentials

You might want to rotate bucket credentials in your account to replace existing credentials with new ones. If you rotate credentials, rotate them during a maintenance window when no backups are in progress (scheduled or on-demand).
...
Remove bucket credentials from an account at any time. You should remove credentials only after unmanaging all clusters.
