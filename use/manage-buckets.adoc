...
You might want to rotate or remove bucket credentials.

If you rotate credentials, rotate them during a maintenance window when no backups are in progress (scheduled or on-demand).

Remove S3 credentials from an account at any time using the Astra Control API. For details about removing credentials, see https://docs.netapp.com/us-en/astra-automation/index.html[Use the Astra Control API^].
