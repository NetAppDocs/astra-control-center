...
IMPORTANT: After enabling the Cloud Insights connection, if you unmange the cluster from Astra Control Center, the monitoring operator deployed on the remote cluster **is not** deleted. If you remove the  backends that you added from Astra Control Center, the **backends are also deleted** from Cloud Insights. 
