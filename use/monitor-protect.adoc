...
.. In Cloud Insights, generate a read key.
+
image:cloud-insights-api.png[Shows the Cloud Insights API token generation page.]
.. Copy the read key. You will need to paste it into the Astra Control Center window for enabling the Cloud Insights connection.
.. In Cloud Insights, generate a write key.
.. Copy the write key. You will need to paste it into the Astra Control Center window for enabling the Cloud Insights connection.
+
NOTE: We recommend that you generate a read key and a write key, and not use the same key for both. By default, the token expiry period is set to one year. We recommend that you keep the default selection to give the token the maximum duration before it expires. If your token expires, the telemetry will stop.
