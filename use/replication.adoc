...
[.lead]
Using Astra Control, you can achieve disaster recovery by configuring app replication to a remote cluster in preparation using NetApp SnapMirror technology. The replication process includes data in persistent volumes replicated by SnapMirror and the app metadata protected by Astra Control.
...
+
The Snapshot name uses the format of "replication-schedule-<string>". Astra Control retains the last Snapshot that was used for replication. Any older replication Snapshots are deleted aftr successful completion of replication.

.Result

This creates the replication relationship.

Astra Control completes following actions as a result of establishing the relationship:
...
* The source cluster and app (of operational) is not stopped and will continue to run.
...
* Astra Control execution hooks (if present) stop any writes to the original source app.
...
This combines the results form "resync" and "reverse relationship" operations to bring the application online on the original source cluster with replication resumed to the original destination cluster.
...
== Replication life cycle state and status

Astra Control displays states of the life cycle of the replication relationship and the health of the relationship itself.
