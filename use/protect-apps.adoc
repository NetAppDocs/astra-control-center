...
[IMPORTANT]
===============================
If your app uses a storage class backed by the `ontap-nas-economy` driver, snapshots can't be created. Use an alternate storage class for snapshots. Astra Trident and Astra Control Provisioner support snapshot creation using the following drivers:
