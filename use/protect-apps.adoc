...
[.lead]
Protect all apps by taking snapshots and backups using an automated protection policy or on an ad-hoc basis. You can use the Astra Control Center UI or https://docs.netapp.com/us-en/astra-automation/index.html[the Astra Control API^] to protect apps.
...
IMPORTANT: If your app uses the `ontap-nas-economy` storage class, protection policies cannot be used. Use an alternate storage class if you want a protection policy. 
...
IMPORTANT: If your app uses the `ontap-nas-economy` storage class, snapshots cannot be used. Use an alternate storage class for snapshots. 
...
IMPORTANT: If your app uses the `ontap-nas-economy` storage class, backups are disruptive and the app will be unavailable until the backup operation has completed. 
