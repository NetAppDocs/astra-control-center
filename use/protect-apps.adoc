...
=== [Tech preview] Create a scheduled backup of namespaces
You can create a scheduled, automatic backup of any or all namespaces on a cluster. These workflows are provided by NetApp as CR files paired with a Python script. When you configure and install the workflows, namespace backups are automatically created at a schedule that you choose during installation. You can protect all namespaces on a cluster with the full cluster backup workflow, or only certain namespaces with the label-based backup workflow.
...
An an example, when you configure and install the full cluster backup workflow, any apps in any namespace are periodically backed up without further effort by the administrator. The namespace doesn't have to exist at the time you install the workflow; if a namespace is added in the future, it will be protected. 
