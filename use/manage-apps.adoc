...
. Select *Discovered*.
+
image:acc_apps_discovered2.png[Screen shot of discovered apps]
...
+
For example, if you want to manage all apps under the "cattle-logging" namespace together so that they have the same snapshot and backup policies, you would manage the namespace and ignore the apps in the namespace.

.	To see the list of managed apps, select *Managed* as the display filter.
+
image:acc_apps_managed2.png[Screen shot of managed apps]
+
...
. Select *Define*.
+
image:acc_apps_custom_details2.png[Screen shot of define custom app]
...
image:screenshot-system-apps.gif[A screenshot that shows the Show System Apps option that is available in the Apps page.]
