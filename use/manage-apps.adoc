...
. Choose one or more namespaces for your application from the *Namespace* drop-down list.
+
...
. (Optional) Enter one or more labels for the Kubernetes resources in each namespace. You can specify a single label or label selector criteria (query).
+
