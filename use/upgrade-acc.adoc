...
.. Prepare and run the following script:

... Substitute <YOUR_REGISTRY> with the URL of the repository.
... For `Load to local cache`, remove the comment format "# " from the appropriate command for the Podman version you are using.
... Run the script.
+
[source,console]
----
...
for astraImageFile in $(ls ${DIRECTORYNAME}/images/*.tar) ; do
  # Load to local cache
  # For Podman 3 releases, use this: 
  # astraImage=$(podman load --input ${astraImageFile} | sed 's/Loaded image(s): //')
  # For Podman 4 releases, use this: 
