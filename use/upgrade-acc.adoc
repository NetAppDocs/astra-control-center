...
** (Local registries only) If you are using a local registry, verify that your image registry path matches the registry path you pushed the images to in a <<Add the images to your local registry,previous step>>. Update `imageRegistry` inside of `spec` if the local registry has changed since your last installation.
