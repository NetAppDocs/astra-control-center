...
// End snippet
//astradoc-249
* Istio service mesh: It is strongly recommended that Astra Control host cluster communications channels be secured using an link:../get-started/requirements.html#service-mesh-requirements[Istio service mesh]. If you already have an existing Astra Control Center instance that is participating in Istio service mesh or you do not intend to install an Istio service mesh for use with Astra Control Center, _no additional changes are needed_ for upgrade configuration. If you intend to install and use an Istio service mesh, _the following steps are required_:
+
[id=istioprereq]
...
+
----
astra.netapp.io/istio: on-upgrade
istio-injection: disabled
----

NOTE: When you perform the upgrade, you will also need to link:../use/upgrade-acc.html#upgrade-astra-control-center[ensure that the `ingressType` you specify in the Astra Control Center CR is `generic`] and not `ACCTraefik` (the default) or the upgrade will fail.
