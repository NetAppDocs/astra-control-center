...
* *The link:../get-started/requirements.html#astra-control-provisioner[Astra Control Provisioner] is enabled and Astra Trident is running*
+
.Expand for steps
[%collapsible]
====
...
NOTE: When you perform the upgrade, you'll also need to ensure that the `ingressType` you specify in link:../use/upgrade-acc.html#upgrade-astra-control-center[the Astra Control Center CR] is `Generic` and not `AccTraefik` or the upgrade will fail.
====

//End snippet

IMPORTANT: Integrating Astra Control Center with a service mesh can only be done during Astra Control Center link:../get-started/install_acc.html[installation] or link:../use/upgrade-acc.html[upgrade] and not independent of these processes. If you integrate the service mesh as part of upgrade, existing application labels will be removed and need to be reapplied. Changing back from a meshed to an unmeshed environment is not supported.
