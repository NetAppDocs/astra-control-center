...
* <<Upgrade Astra Control Center>>
* <<(Optional) Upgrade third-party services>>
...
.. Change the Astra version (`astraVersion` inside of `Spec`) number to the latest:
+
----
...
Upgrades to Traefik and Cert-manager are handled a bit differently.
...
+
On the other hand, if you have your own Traefik deployment, set `externalTraefik` to `true`. In this case, you maintain the deployment and Astra Control Center will upgrade the CRDs.
