...
. Add the following to your `CRDs` configuration inside of `Spec` if you are using your own ingress controller (`ingressType: "Generic"` in the Astra Control Center CR):
+
----
crds:
  shouldUpgrade: false
----
+
NOTE: `shouldUpgrade` must be set to `true` only if you used `ingressType: "AccTraefik"` during Astra Control Center installation.
...
----
+
NOTE: To monitor upgrade status during the operation, run the following command: `kubectl get -o yaml -n [netapp-acc or custom namespace] acc`
