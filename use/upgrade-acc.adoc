...
// End snippet
+
NOTE: Upgrade Astra Trident, if required, using these link:../use/upgrade-trident.html[instructions^]. 
...
. If you installed a new Istio service mesh on the host cluster according to the <<istio-prereq,prerequisite steps>> described earlier in this procedure, change the ingress setting to `Generic`:
+
[source,console]
----
ingressType: Generic
