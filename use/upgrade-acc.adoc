...
+
IMPORTANT: Don't attempt to add the mesh to the existing Astra installation. The Istio service mesh must be running in parallel with an existing, unmeshed Astra Control Center installation. Existing Istio application labels will be removed and need to be reapplied in the next step.

//last sentence needs verification from Brett
