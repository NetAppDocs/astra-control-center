...
NOTE: The application cluster should be ideally situated outside of this three-cluster scenario described here. 
...
... Change the `astraAddress` under `spec:` with the IP address or FQDN (`ocp-cluster-3.company.com`) from the destination third cluster.
