...
.. Enable the snapshot directory for the PVC associated with the application:
+
[source,console]
----
...
.Result
The application is ready to backup and restore without any disruptions using Astra Control. The PVC is also available to be used by other applications for non-disruptive backups and restores.
====
// End snippet
