...
== Enable non-disruptive backup and restore for ontap-nas-economy operations

Astra Control Provisioner provides non-disruptive backup and restore functionality that can be enabled for storage backends that are using the `ontap-nas-economy` storage class.
...
.. Enable the Snapshot directory for the PVC associated with application:
+
[source,console]
----
tridentctl update volume <pvc name> --snapshot-dir=true --pool-level=true -n trident
----
...
[source,console]
----
tridentctl get volume <pvc name> -n trident -oyaml | grep snapshotDir
----
+
Response:
+
...
.. Log in to a ONTAP terminal where the volumes are created.
...
.Result
The application is ready to backup and restore without any disruptions. The PVC is also available to be used by other applications for non-disruptive backups and restores.
====
// End snippet
