...
+
NOTE: You must specify at least one namespace, label, or GVK.
...
.. (Optional) *Label selector*: Include a https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors[label selector^] to add to the rule. The label selected is used to filter only those resources matching this label.
...
... *Version*: Enter the Kubernetes API version.
