...
NOTE: If your app uses the `ontap-nas-economy` storage class, you must restore the app using the original storage classes. You cannot specify a different storage class if you are restoring the app to the same namespace.
...
+
NOTE: You can use this procedure to either <<Migrate from ontap-nas-economy storage to ontap-nas storage,migrate an app from an `ontap-nas-economy` storage class>> to an `ontap nas` storage class on the same cluster *OR* copy the app to another cluster backed by the `ontap-nas-economy` storage class. 
