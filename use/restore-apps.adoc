...
* *Filter resources*: Specify rules to restore only a sub-set of the original application resources.
... Choose to include or exclude resources from the restored application.
... START HERE!!!  Set the rule:

You can add a rule that will include resources of a specific group, kind, version and optionally, label. You might want to do this if there are resources that Astra Control does not include automatically. 
