...
IMPORTANT: Astra Control Center uses the email address in either the LDAP `mail` or `userPrincipalName` attributes to search for and keep track of remote users. These attributes might be an optional or empty field in your directory. An email address must exist in one of these fields for any remote users you wish to appear in Astra Control Center. This email address is used as the username in Astra Control Center for authentication.
...
+
For example, some Microsoft Active Directory systems might use `userPrincipalName` as the login attribute.
