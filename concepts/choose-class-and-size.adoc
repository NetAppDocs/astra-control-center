...
When you use Astra Control Center integrated with an Astra Data Store storage backend, after the installation, no storage classes are defined. You will need to create the Trident default storage class and apply it to the storage backend. See https://review.docs.netapp.com/us-en/astra-data-store_ads-PI4-review/get-started/setup-ads.html#set-up-astra-data-store-as-storage-backend[Astra Data Store getting started] to create a default Astra Data Store storage class.
