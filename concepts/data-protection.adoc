...
image:acc-backup_4in.png["Image shows a scheduled backup and restore between two K8s clusters using S3 bucket"]

On the other hand, replication is done by replicating to ONTAP and then a fail over creates the Kubernetes resources:

image:acc-replication_4in.png["Image shows replication and fail over between two K8s clusters and two ONTAP clusters "]
