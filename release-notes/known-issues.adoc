...
* <<Upgrade fails if source version uses a container image registry that doesn't require authentication and target version uses a container image registry that requires authentication>>
...
----
. Proceed with the upgrade using the instructions link:../use/upgrade-acc.html#add-the-images-to-your-local-registry[here^].
