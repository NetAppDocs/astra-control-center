...
== The same cluster cannot be managed by two Astra Control Center instances
//DOC-3600/Q2/PI4/PI5
If you want to manage a cluster on another Astra Control Center instance, you should first link:../use/unmanage.html#unmanage-a-cluster[unmanage the cluster] from the instance on which it is managed before you manage it on another instance. After you remove the cluster from management, verify that the cluster is unmanaged by executing this command:
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html[Install the latest Astra Trident version^].
...
Ensure that you link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site[enter the correct values] when establishing a connection.
