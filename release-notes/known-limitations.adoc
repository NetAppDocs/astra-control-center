...
* <<Label selectors should be updated if a namespace uses the same name as a namespace previously removed in Astra Control>>
...
You will not be able to define and manage an application on the new namespace with Astra Control if the associated label selector requirements (for example, `app in (mysql, mariadb)`) are the same as the label selector requirements associated with the removed namespace. You should first unmanage the old application and then define a new app with different requirements for the label selector (for example, `app in (mariadb, jenkins)`).
