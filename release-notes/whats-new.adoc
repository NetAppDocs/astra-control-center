...
• API implementation for namespace RBAC
• RBAC credential change for namespace scoping
• Hybrid access for private clusters in Azure Kubernetes Service (AKS)
• Bucket removal:
− Remove a bucket from Astra Control
− Namespaces
− Advanced namespace lifecycle
− Namespace scoping authorization and filtering
− Generic ingress
• Enable a generic ingress resource to be consumed by various ingress controllers
• MongoDB
• Enable secure logins to MongoDB
