...
* Enhanced data protection that now supports adding cluster-scoped resources in your app namespaces. You can manage cluster resources that are associated with namespaced resources in addition to those that Astra Control includes automatically. 
* Enhanced support for LDAP authentication with role-based access control (RBAC). 
