...
* Minor bug fixes.
* Deploy Astra Control Center without a private registry.
...
[%collapsible]
====
This CR-based data management functionality is provided as a tech preview and can be accessed only when you install the link:../get-started/install-astra-connector.html[Astra Connector] on the cluster you want to manage. After you link:../get-started/add-cluster.html[add the cluster manually] to Astra Control using the instructions in the UI, you'll be able to perform the following CR-based cluster operations in the UI or from a CR:

* link:../use/manage-apps.html#tech-preview-define-an-application-using-a-kubernetes-custom-resource[Define an application using a custom resource] and link:../use/manage-buckets.html#tech-preview-manage-a-bucket-using-a-custom-resource[define its bucket].
