...
* Enhanced replication protection to a remote system using SnapMirror technology.  
* Enhanced data protection that now supports managing applications that are defined across multiple namespaces and adding cluster resources that are associated with namespaced resources in addition to those that Astra Control includes automatically. 
* Enhanced LDAP authentication with role-based access control (RBAC) integrations. 
