...
During Astra Control Center installation, if you specify `enrolled: true` for `autoSupport` in the Astra Control Center Custom Resource Definition (CRD) file (`astra_control_center_min.yaml`), daily support bundles are automatically uploaded to NetApp Support.
...
NOTE: The time taken by Astra Control Center to generate the bundle depends on the size of your Astra Control Center installation as well as the bundle itself. The time duration that you set for Astra Control Center to generate a bundle dictates the time it takes for the bundle to be generated (for example: A shorter time period means faster bundle generation).
